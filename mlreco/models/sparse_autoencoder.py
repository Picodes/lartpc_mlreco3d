...
class EncoderModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        encoding, image, original,encoder_x,decoder_x, dec_sparse_x = self.encoder(data, clusts)
...
def compare_sparse(x, y):
    ...
    if cR.numel():
        ...
        e += 1/3*torch.max(1-y.features[cR,0],torch.zeros(y.features[cR,0].size())).pow(2).sum()/cR.numel()/2
    if R.numel():
        e += 2/3*torch.max(1+y.features[R,0],torch.zeros(y.features[R,0].size())).pow(2).sum()/R.numel()/2
    return e
