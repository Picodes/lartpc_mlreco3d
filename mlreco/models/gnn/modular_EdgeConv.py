    ...
    def __init__(self, cfg, name='full_gnn'):
        ...
        for i in range(self.num_mp):
            self.edge_mlps.append(
                Seq(
                    BatchNorm1d(2*nInput),
                    Lin(2*nInput, 2*nInput),
                    LeakyReLU(self.leakiness),
                    BatchNorm1d(2*nInput),
                    Lin(2*nInput, nOutput)
                )
            )
            ...
            self.edge_updates.append(
                MetaLayer(EdgeLayer(nOutput, self.edgeInput, self.edgeInput,
                          ...
                          NodeLayer(nOutput, nOutput, self.edgeInput, leakiness=self.leakiness)
                         )
            )
            nInput = nOutput
